from stemseg.modeling.embedding_utils import add_glow_offset, add_spatiotemporal_offset, \
    get_nb_embedding_dims, get_nb_free_dims
...
class SqueezingExpandDecoder(nn.Module):
    def __init__(self, in_channels, inter_channels, embedding_size, tanh_activation,
        ...
        # offset 32x -> 16x
        self.off_upsample_32_to_16 = nn.Sequential(
            UpsampleTrilinear3D(scale_factor=(1, 2, 2), align_corners=False)
        )
        # offset 16x to 8x
        self.off_upsample_16_to_8 = nn.Sequential(
            UpsampleTrilinear3D(scale_factor=(1, 2, 2), align_corners=False)
        )
        # offset 8x to 4x
        self.off_upsample_8_to_4 = nn.Sequential(
            UpsampleTrilinear3D(scale_factor=(1, 2, 2), align_corners=False)
        )

    def forward(self, x, offset_var):
        """
        ...
        if offset_var is not None:
            embeddings = add_glow_offset(embeddings, offset_var, self.time_scale, self.embedding_dim_mode)
        else:
