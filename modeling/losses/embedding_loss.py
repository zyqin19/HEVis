...
class EmbeddingLoss(nn.Module):
    def __init__(self, embedding_map_scale, **kwargs):
        ...
        assert len(kwargs["free_dim_stds"]) == self.n_free_dims, \
            "List of std values {} does not match number of free dims {}, \r\n i.e. the dim of cfg.TRAINING.LOSSES.EMBEDDING" \
            ".FREE_DIM_STDS {} should be equal cfg.MODEL.GLOW_DIM_MODE {}".format(len(kwargs["free_dim_stds"]),
            self.n_free_dims, cfg.TRAINING.LOSSES.EMBEDDING.FREE_DIM_STDS, cfg.MODEL.GLOW_DIM_MODE)
    ...
    def forward(self, embedding_map, targets, output_dict, *args, **kwargs):
        ...
        if cfg.MODEL.USE_OFFSET_HEAD:
            ...
            # output_dict[ModelOutputConsts.OPTIMIZATION_LOSSES][LossConsts.EMBEDDING] = total_loss * self.w
            output_dict[ModelOutputConsts.OTHERS][LossConsts.LOVASZ_LOSS] = lovasz_loss
            output_dict[ModelOutputConsts.OTHERS][LossConsts.VARIANCE_SMOOTHNESS] = bandwidth_smoothness_loss
        else:
            output_dict[ModelOutputConsts.OPTIMIZATION_LOSSES] = {
                LossConsts.EMBEDDING: total_loss * self.w
            }
