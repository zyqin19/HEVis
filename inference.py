...
class TrackGenerator(object):
    def __init__(self, sequences, dataset_name, output_generator, output_dir, model_ckpt_path, cpu_workers,
                 max_tracks, preload_images, resize_scale, semseg_averaging_on_gpu, **kwargs):
        ...
        self.model = InferenceModel(cfg, model_ckpt_path, cpu_workers=cpu_workers, semseg_output_type=semseg_output_type,
                                    preload_images=preload_images, resize_scale=resize_scale,
                                    semseg_generation_on_gpu=semseg_averaging_on_gpu).cuda()
...
def main(args):
    ...
    track_generator = TrackGenerator(
        ...
        cpu_workers=args.num_cpu_workers,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_path', type=str, default='E:\PythonProjects\STEm-Seg-data\stemsegmodels\checkpoints\\'
                                                                'ytvis\ytvis_v8\glow_youtube_151_v8_12.31\\017000.pth')

    # parser.add_argument('--output_dir', '-o',              required=False)
    parser.add_argument('--output_dir', '-o', type=str, default='E:\PythonProjects\STEm-Seg-data\output\\'
                                                                'ytvis\ytvis_v8\glow_youtube_151_v8_12.31\\017000_Win')
    parser.add_argument('--seqs',       nargs="*",         required=False)
    # parser.add_argument('--dataset',    '-d',              required=True)
    parser.add_argument('--dataset', '-d', type=str, default='ytvis')
    ...
    parser.add_argument('--resize_embeddings', default=False)
